...
class AbilityViewModel: ObservableObject {
    ...
    private var database = ConstantsController.shared
    ...
    init(scepter: String, shard: String, ability: String) {
        ...
        guard let scepterURL = URL(string: scepter),
            ...
            let abilityURL = URL(string: ability)
        else {
            return
        }
    ...
    private func setupBinding() {
        ...
        $opentDotaAbility
            .sink { [weak self] ability in
                ...
                guard
                    let parsedImageURL = ability?
                        .img?
                        .replacingOccurrences(of: "_md", with: "")
                        .replacingOccurrences(
                            of: "images/abilities",
                            with: "images/dota_react/abilities")
                else {
                    return
                }
    ...
    private func getVideoURL(_ ability: String, type: ScepterType) async -> AVAsset? {
        guard
            let heroName = try? database.fetchHeroWithID(id: heroID).name.replacingOccurrences(
                of: "npc_dota_hero_", with: "")
        else {
            return nil
        }
