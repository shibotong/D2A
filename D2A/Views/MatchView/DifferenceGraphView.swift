        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack {
                VStack(alignment: .leading, spacing: 3) {
                    Text("\(Int(vm.mins)):00").font(.system(size: 10)).bold().foregroundColor(
                        Color(.secondaryLabel))
                    HStack {
                        Circle().frame(width: 10, height: 10).foregroundColor(.yellow)
                        Text("\(abs(vm.goldDiff[Int(vm.mins)]))").foregroundColor(
                            Color(vm.goldDiff[Int(vm.mins)] >= 0 ? .systemGreen : .systemRed)
                        ).font(.system(size: 15)).bold()
                    }
                    HStack {
                        Circle().frame(width: 10, height: 10).foregroundColor(.blue)
                        Text("\(abs(vm.xpDiff[Int(vm.mins)]))").foregroundColor(
                            Color(vm.xpDiff[Int(vm.mins)] >= 0 ? .systemGreen : .systemRed)
                        ).font(.system(size: 15)).bold()
                    }
                }
                Spacer()
            ...
            ZStack {
                ...
                DrawDiffLines(data: vm.xpDiff, max: fetchLargestABS(), selectedTime: Int(vm.mins))
                    .foregroundColor(.blue).clipped().shadow(radius: 1)
                DrawDiffLines(data: vm.goldDiff, max: fetchLargestABS(), selectedTime: Int(vm.mins))
                    .foregroundColor(.yellow).clipped().shadow(radius: 1)
                ...
                CurrentPoint(data: vm.xpDiff, max: fetchLargestABS(), selectedTime: Int(vm.mins))
                    .foregroundColor(.blue).animation(.linear(duration: 0.1), value: vm.mins)
                CurrentPoint(data: vm.goldDiff, max: fetchLargestABS(), selectedTime: Int(vm.mins))
                    .foregroundColor(.yellow).animation(.linear(duration: 0.1), value: vm.mins)
            ...
            HStack {
                ...
                Text("\(Int(vm.goldDiff.count - 1)):00").font(.system(size: 15)).foregroundColor(
                    Color(.secondaryLabel))
            }.padding()
            Spacer()
        }
...
struct DifferenceGraphView_Previews: PreviewProvider {
    static var previews: some View {
        DifferenceGraphView(vm: DifferenceGraphViewModel(goldDiff: [1, 2, 3], xpDiff: [4, 5, 6]))
