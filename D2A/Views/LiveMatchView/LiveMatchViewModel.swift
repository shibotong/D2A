...
//    }
    ...
    private func startSubscription() {
        let subscription = ApolloNetwork.shared.apollo.subscribe(subscription: LiveMatchSubscription(matchid: matchID)) { [weak self] result in
            switch result {
    ...
    private func fetchHistoryData(lastFetchTime: Int) {
        let subscription = ApolloNetwork.shared.apollo.fetch(query: LiveMatchHistoryQuery(matchid: matchID), cachePolicy: .fetchIgnoringCacheCompletely) { [weak self] result in
            switch result {
            case .success(let graphQLResult):
