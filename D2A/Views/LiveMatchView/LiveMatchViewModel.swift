...
class LiveMatchViewModel: ObservableObject {
  ...
  private func startSubscription() {
    let subscription = Network.shared.apollo.subscribe(
      ...
      case .success(let graphQLResult):
        ...
        if let radiantTeamId = graphQLResult.data?.matchLive?.radiantTeamId,
          let radiantTeam = self?.radiantTeam, radiantTeam.isEmpty
        {
          self?.radiantTeam = radiantTeamId.description
        }
        if let direTeamId = graphQLResult.data?.matchLive?.direTeamId,
          let direTeam = self?.direTeam, direTeam.isEmpty
        {
        ...
        if let liveMatchPlayers = graphQLResult.data?.matchLive?.players {
          let heroes: [LiveMatchHeroPosition] = liveMatchPlayers.compactMap { player in
            ...
            return LiveMatchHeroPosition(
              heroID: Int(heroID), isRadiant: heroIsRadiant, xPos: CGFloat(xPos),
              yPos: CGFloat(yPos))
          }
          ...
          for player in liveMatchPlayers {
            ...
            players.append(
              Player(
                heroID: Int(heroID), isRadiant: isRadiant, deathEvents: deathEvent,
  ...
  private func fetchHistoryData(lastFetchTime: Int) {
    let subscription = Network.shared.apollo.fetch(
      ...
      switch result {
      case .success(let graphQLResult):
        if let gameMode = graphQLResult.data?.live?.match?.gameMode,
          gameMode == .captainsMode || gameMode == .captainsDraft
        {
          self?.hasBan = true
        } else {
          self?.hasBan = false
          ...
          .buildingEvents
        {
          let events: [LiveMatchBuildingEvent] = buildingEventsData.compactMap { event in
            guard let event,
              let buildingID = event.indexId,
              let isRadiant = event.isRadiant,
              let type = event.type, event.time > lastFetchTime
            else {
              return nil
            }
            return LiveMatchBuildingEvent(
        ...
        if let liveMatchPlayers = graphQLResult.data?.live?.match?.players {
          ...
          for player in liveMatchPlayers {
            ...
            let deathEvent: [Int] =
              player.playbackData?.deathEvents?.compactMap { $0?.time }.filter {
                $0 > lastFetchTime
              } ?? []
            let killEvent: [Int] =
