//
//  RecentMatchMigrationPolicy.swift
//  D2A
//
...
class RecentMatchMigrationPolicy: NSEntityMigrationPolicy {
    
    private let sourceKeyID = "id"
    private let destinationKeyID = "matchID"
    private let sourceKeyUserID = "playerId"
    private let destinationKeyUserID = "userID"
    
    override func begin(_ mapping: NSEntityMapping, with manager: NSMigrationManager) throws {
        logDebug("Start mapping RecentMatch...", category: .coredata)
    }
    ...
    override func createDestinationInstances(forSource sInstance: NSManagedObject, in mapping: NSEntityMapping, manager: NSMigrationManager) throws {
        ...
        for key in keys {
            if key != sourceKeyID && key != sourceKeyUserID { // skip transformed field
        ...
        if let sourceUserID = sInstance.value(forKey: sourceKeyUserID) as? String {
            let idInt = Int64(sourceUserID)
            dest.setValue(idInt, forKey: destinationKeyUserID)
        }
