...
class HeroMigrationPolicy: NSEntityMigrationPolicy {
    
    private let sourceKeyID = "id"
    private let destinationKeyID = "heroID"
    
    override func begin(_ mapping: NSEntityMapping, with manager: NSMigrationManager) throws {
        logDebug("Start mapping hero", category: .coredata)
    }
    ...
    override func createDestinationInstances(forSource sInstance: NSManagedObject, in mapping: NSEntityMapping, manager: NSMigrationManager) throws {
        ...
        if let sourceId = sInstance.value(forKey: sourceKeyID) as? Double {
            let idInt = Int16(sourceId)
